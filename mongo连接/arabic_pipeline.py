...
import pymongo,urllib.parse,os,pexpect,json
    ...
    def __init__():
        conn = pymongo.MongoClient(
            "mongodb://{user}:{password}@{host1}:{port1},{host2}:{port2},{host3}:{port3}/{database}".format(user=user,password=urllib.parse.quote_plus(password),
                ...
                port3=port3,
                database=database))
        filename='full_arabic.dat'

    def parse():
        num=conn.arabic_search.article.find({"nlp_state":1,"state_engine":{'$exists': False}})
