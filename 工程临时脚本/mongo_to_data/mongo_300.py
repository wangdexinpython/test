...
class Mongo_to_data(object):
    def __init__():
        ...
        f = open(r'./article_300_2.dat','a+',encoding='utf-8')
    def parse():
        # num = mycol.find({'nlp_state':1},{'_id':0}).sort([('time_stamp',-1)]).limit(90000)
        # print('num',num)
        for i in range(0,90000,300):
        #     print('i',i)
            zds = mycol.find({'nlp_state':1},{'_id':0}).skip(i).limit(1)
            for m in zds:
            #print('m',m)
