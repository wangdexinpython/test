...
class BloomFilter(object):
    ...
    def __init__(, capacity, error_rate=0.001):
        ...
        _setup(error_rate, num_slices, bits_per_slice, capacity, 0)
        bitarray = bitarray.bitarray(num_bits, endian='little')
        bitarray.setall(False)

    def _setup(, error_rate, num_slices, bits_per_slice, capacity, count):
        ...
        count = count
        make_hashes = make_hashfuncs(num_slices, bits_per_slice)
    ...
    def add(, key, skip_check=False):
        ...
        bits_per_slice = bits_per_slice
        hashes = make_hashes(key)
        if not skip_check and hashes in :
            return True
        if count > capacity:
    ...
    def copy():
        ...
        """
        new_filter = BloomFilter(capacity, error_rate)
        new_filter.bitarray = bitarray.copy()
        return new_filter

    def union(, other):
        """ Calculates the union of the two underlying bitarrays and returns
        a new bloom filter object."""
        if capacity != other.capacity or \
...
have equal capacity and error rate")
    ...
    def tofile(, f):
        ...
        f.write(pack(FILE_FMT, error_rate, num_slices,
                     bits_per_slice, capacity, count))
        bitarray.tofile(f)
...
class ScalableBloomFilter(object):
    ...
    def __init__(, initial_capacity=100, error_rate=0.001,
        ...
        _setup(mode, 0.9, initial_capacity, error_rate)
        filters = []

    def _setup(, mode, ratio, initial_capacity, error_rate):
        scale = mode
        ratio = ratio
    ...
    def add(, key):
        ...
        if key in :
            return True
        filter = filters[-1] if filters else None
        if filter is None or filter.count >= filter.capacity:
            num_filters = len(filters)
            filter = BloomFilter(
                capacity=initial_capacity * (scale ** num_filters),
                error_rate=error_rate * (ratio ** num_filters))
    ...
    def capacity():
        ...
        return sum([f.capacity for f in filters])
    ...
    def tofile(, f):
        ...
        `f'."""
        f.write(pack(FILE_FMT, scale, ratio,
                     initial_capacity, error_rate))

        # Write #-of-filters
        f.write(pack('<l', len(filters)))

        if len(filters) > 0:
            # Then each filter directly, with a header describing
            # their lengths.
            headerpos = f.tell()
            headerfmt = '<' + 'Q'*(len(filters))
    ...
    def __len__():
        ...
        return sum([f.count for f in filters])
